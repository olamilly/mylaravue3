<script setup>
    ...
    const { users, getUsers, registerForm, newUser, updateForm, updateUser, deleteUser, delid } = usePeople()
    const createUser = ()=>{
        newUser();
        getUsers();
    }
    const allowSubmit=()=>{
        if(registerForm.password === registerForm.passwordConfirm && registerForm.password.length >= 8 && registerForm.name!="" && registerForm.email!=""){
 ...
 <table class="table">
        ...
        <tbody>
                ...
                <tr v-for="(user, index) in users" :key="user.id">
                    ...
                    <td class=td>
                        ...
                        <a class="btn btn-danger mx-1 btn-sm" id="deleteBtn" data-toggle="modal" @click="modalControl($event, user)" data-target="#exampleModal">Delete User</a>
                    </td>
                </tr>
    ...
    <div class="modal fade" id="exampleModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
      ...
      <div class="modal-footer">
        ...
        <button type="submit" id="form2Submit" class="btn btn-primary" data-dismiss="modal"  @click="editUser" >Update</button>
      </div>
    </div>
            </div>
        </div>
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
        <form>
            <div class="row mb-3">
                <label for="name" class="col-md-4 col-form-label text-md-end">Name</label>
                <div class="col-md-6">
                    <input type="text" v-model="registerForm.name" name="name" class="form-control" required autofocus>
                </div>
            ...
            <div class="row mb-3">
                <label for="email" class="col-md-4 col-form-label text-md-end">Email Address</label>
                <div class="col-md-6">
                    <input type="email" v-model="registerForm.email" class="form-control" name="email"  required>
                </div>
            </div>
            ...
            <div class="row mb-3">
                ...
                <div class="col-md-6">
                    <input id="password" type="password" v-model="registerForm.password" class="form-control" name="password" required >
                </div>
            </div>
            ...
            <div class="row mb-3">
                ...
                <div class="col-md-6">
                    <input id="password-confirm" type="password" class="form-control" v-model="registerForm.passwordConfirm" name="password_confirmation" required >
                </div>
            </div>
        </form>
      </div>
      <div class="modal-footer">
        ...
        <button type="submit" id="formSubmit" disabled class="btn btn-primary" data-dismiss="modal" @click="createUser">Submit</button>
      </div>
    </div>
  </div>
</div>
